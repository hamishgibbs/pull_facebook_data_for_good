...
@click.option('-o', '--outdir', help='Outfile directory. Default: current directory.', default = os.getcwd())
@click.option('-e', '--end_date', help='Dataset end date. Default: datetime.now().', default = datetime.now())
@click.option('-f', '--frequency', help='Dataset update frequency (hours). Default: 8.', default = 8)
@click.option('-driver', '--driver_path', help='Path to webdriver.', default = '/Applications/chromedriver')
def cli(dataset_name,
    ...
    data_dates = utils.get_file_dates(config['start_date'],
                                      config['end_date'],
                                      frequency)
